...
function initializeProjectsCarousel() {
    ...
    function getPreviousIndex(index) { return (index - 1 + slides.length) % slides.length; }
    function getNextIndex(index) { return (index + 1) % slides.length; }
    function updateCarouselState(index) {
        ...
        slides.forEach((slide, slideIndex) => {
            ...
            if (slideIndex === currentIndex) slide.classList.add('is-active');
            else if (slideIndex === previousIndex) slide.classList.add('is-prev');
            else if (slideIndex === nextIndex) slide.classList.add('is-next');
            else slide.classList.add('is-hidden');
        });
        prevButton.disabled = false;
        nextButton.disabled = false;
    }
    function scrollToSlide(index) { updateCarouselState(index); }
    ...
    track.addEventListener('keydown', function(event) {
        if (event.key === 'ArrowLeft') { event.preventDefault(); scrollToSlide(currentIndex - 1); }
        if (event.key === 'ArrowRight') { event.preventDefault(); scrollToSlide(currentIndex + 1); }
    });
    slides.forEach((slide, slideIndex) => {
        slide.addEventListener('click', function() { if (slideIndex !== currentIndex) scrollToSlide(slideIndex); });
    });
...
function initializeObserverAnimations() {
    ...
    const observer = new IntersectionObserver(function(entries) {
        entries.forEach(entry => {
            if (entry.isIntersecting) { animateElement(entry.target); observer.unobserve(entry.target); }
        });
    }, observerOptions);
    const elementsToAnimate = document.querySelectorAll('.project-card, .skill-category, .soft-skill-item, .stat-card, .timeline-item, .contact-form, .contact-info');
